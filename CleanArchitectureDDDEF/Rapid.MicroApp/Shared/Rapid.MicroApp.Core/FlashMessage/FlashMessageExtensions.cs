...
namespace Rapid.MicroApp.Core.FlashMessage
{
    public static class FlashMessageExtensions
    {
        ...
        public static void QueueWithKey(this IFlashMessage flashMessage, FlashMessageType messageType, string keyName, string message, bool isHtml = false)
        {
            // Append the new message.
            var messageModel = new FlashMessageModel { IsHtml = isHtml, KeyName = keyName, Message = message, Type = messageType };
            flashMessage.Queue(messageModel);
        }
        /// <summary>
        ...
        public static void InfoWithKey(this IFlashMessage flashMessage, string keyName, string message, string title = null)
        {
            flashMessage.QueueWithKey(FlashMessageType.Info, keyName, message);
        }
