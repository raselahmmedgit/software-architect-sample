...
var conn = builder.Configuration["AppDb__Connection"] ?? builder.Configuration.GetConnectionString("DefaultConnection") ?? "Server=sqlserver;Database=AppDb;User Id=sa;Password=Your_password123;TrustServerCertificate=True;";
builder.Services.AddDbContext<PaymentDbContext>(options => options.UseSqlServer(conn));
